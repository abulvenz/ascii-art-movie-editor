...
const createProject = function(incoming) {
    ...
    return {
        ...
        addFrame: function() {
            frames.push(
                frames.length > 0 ?
                clone(frames[currentFrameIdx]) : "Welcome\nIn\nThe\nAsciiWorld"
            );
...
const playback = function(vnode) {
    ...
    const playback = () => playbackTimeID = setInterval(() => {
        playbackFrameIdx = (++playbackFrameIdx) % frames.length;
        m.redraw();
    }, playbackSpeed_);
    const playback_stop = () => playbackTimeID = clearInterval(playbackTimeID);
    const playback_stepback = () => playbackFrameIdx = playbackFrameIdx - 1 + (playbackFrameIdx < 1 ? frames.length : 0);
        ...
        view: function({ attrs, showControls = false }) {
            ...
            return div(
                pre(
                    ...
                    showControls ? null : button({
                        style: "float:right;",
                        onclick: () => hidden = !hidden
                    }, !hidden ? '^' : 'v')), //
                !hidden ? [div.buttonGroup(
                    ...
                    input({ type: "number", value: playbackSpeed(), oninput: e => playbackSpeed(Number(e.target.value)) }),
                ] : null
            );
        }
    };
...
if (!queryStringFrames) {
    ...
    m.mount(document.body, {
        view: (vnode) => [
            ...
            input({ value: project.projectID(), oninput: e => project.projectID(e.target.value) }), br(),
            textarea.$screen({
                rows: project.rows(),
                cols: project.cols(),
            ...
            }), project.currentFrameIdx() + 1, '|', project.frames().length,
            br(),
            div.buttonGroup(
    ...
    m.mount(document.body, {
        view: (vnode) => [
            m(playback, { showControls: true, autoplay: true, frames: () => project.frames() })
        ]
    });
